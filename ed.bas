...
190 go to 10
200 rem NOTE "a" command
210 line = line + 1
220 go sub 450
230 input buffer$(line)
240 return
250 rem NOTE "a" command with inline content
260 line = line + 1
270 go sub 450
280 buffer$(line) = mid$(cmd$, 3)
290 return
300 rem NOTE "c" command
310 input buffer$(line)
320 return
350 rem NOTE "c" command with inline content
380 buffer$(line) = mid$(cmd$, 3)
390 return
400 rem NOTE "d" command
410 for i = line to 999
420     buffer$(i) = buffer$(i+1)
430 next i
440 return
450 rem NOTE "d", but backwards, freeing space
460 for i = 999 to line step -1
470     buffer$(i) = buffer$(i-1)
480 next i
490 return
500 rem NOTE "i" command
510 go sub 450
520 input buffer$(line)
530 return
550 rem NOTE "i" command
560 go sub 450
570 buffer$(line) = mid$(cmd$, 3)
580 return
600 rem NOTE "j" command
610 buffer$(line) = buffer$(line) + buffer$(line + 1)
620 line = line + 1
630 go sub 400
640 line = line - 1
650 return
700 rem NOTE "k" command
710 marks(asc(mid$(cmd$, 2))) = line
720 return
800 rem NOTE "m" command
